# This file creates a database in SQL and loads the .xml in a table in SQL
...
def create_table_from_df(df, table_name):
    ...
    for col in df.columns:
        ...
        cols.append(f"`{col}` {sql_type}")  # backticks for safety
...
def insert_data(df, table_name):
    ...
    cols = ",".join([f"`{col}`" for col in df.columns])  # backticks for safety
